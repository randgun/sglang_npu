...
class AscendAttnBackend(AttentionBackend):
            ...
            if self.use_fia:
                ...
                if (
                    ...
                    if not self.use_alibi:
                        ...
                        extend_seq_lens_npu = self.forward_metadata.extend_seq_lens_cpu_int.npu()
                        ...
                        if not forward_batch.forward_mode.is_idle():
                            ...
                            print(f"+++ {rank=} {layer.layer_id=} {self.forward_metadata.block_tables.shape=}, {self.forward_metadata.seq_lens_cpu_int=}, {self.forward_metadata.extend_seq_lens_cpu_int=}, {max_seq_len=}")

                            for i, req_blocks in enumerate(self.forward_metadata.block_tables):
                                tmp_k = k_cache[req_blocks, ...].reshape(-1, layer.tp_k_head_num * layer.qk_head_dim)[:max_seq_len]
                                tmp_v = v_cache[req_blocks, ...].reshape(-1, layer.tp_k_head_num * layer.qk_head_dim)[:max_seq_len]
                                kk.append(tmp_k)
                                vv.append(tmp_v)
                            
                            kk = torch.stack(kk, dim=0)
                            vv = torch.stack(vv, dim=0)
                            k_quant, k_scale = torch_npu.npu_dynamic_quant(kk.view(-1, layer.tp_k_head_num * layer.qk_head_dim))
                            v_quant, v_scale = torch_npu.npu_dynamic_quant(vv.view(-1, layer.tp_v_head_num * layer.v_head_dim))
                            
                            print(f"prefill, {rank=}, {layer.layer_id=} {k_scale=}, {v_scale=}, {k_quant.sum()=}, {v_quant.sum()=}, {k_quant[:, :5]=}, {v_quant[:, :5]=}", flush=True)
    ...
    def forward_mtp(
        ...
        if not self.use_mla:
            ...
            # TODO 算子在Q_S>1时不支持page attention
    ...
    def forward_decode_graph(
        ...
        if not self.use_mla:
            ...
            rank = torch.distributed.get_rank()
            if envs.SGLANG_NPU_PD_ENABLE_C8.get():
                max_seq_len = max(actual_seq_len_kv)
        ...
        if not self.use_mla:
            ...
            if save_kv_cache and k is not None and v is not None:
                ...
                if envs.SGLANG_NPU_PD_ENABLE_C8.get():
                    k_quant, k_dynamic_scale = torch.ops.npu.npu_dynamic_quant(k.view(-1, layer.tp_k_head_num * layer.qk_head_dim))
                    v_quant, v_dynamic_scale = torch.ops.npu.npu_dynamic_quant(v.view(-1, layer.tp_v_head_num * layer.v_head_dim))
                    print(f"++++ {forward_batch.out_cache_loc.shape=}, {k_dynamic_scale=}, {v_dynamic_scale=} {k_dynamic_scale.dtype=} {forward_batch.out_cache_loc=}")
                    forward_batch.token_to_kv_pool.set_kv_buffer(
                        layer,
                        forward_batch.out_cache_loc,
                        k_quant.view(-1, layer.tp_k_head_num, layer.qk_head_dim),
            ...
            rank = torch.distributed.get_rank()
            if not forward_batch.forward_mode.is_idle():
                print(f"+++ {rank=}, {layer.layer_id=}, {k_cache.data_ptr()=}, {v_cache.data_ptr()=}")
            ...
            if self.use_fia:
                ...
                if envs.SGLANG_NPU_PD_ENABLE_C8.get():
                    ...
                    if not forward_batch.forward_mode.is_idle():
                        print(f"+++ {rank=}, {layer.layer_id=} {self.forward_metadata.seq_lens.shape=} {self.forward_metadata.seq_lens_cpu_int=} {self.forward_metadata.block_tables=}", flush=True)
                        print(f"+++ {rank=}, {layer.layer_id=} {kv_dequant_scale.shape=}, {self.forward_metadata.block_tables.shape=}, {kv_dequant_scale=}, {kv_dequant_scale.sum()=}, {kv_dequant_scale.data_ptr()=}", flush=True)
                        print(f"+++ {rank=}, {layer.layer_id=}, before attention {q.sum()=}, {q.shape=} {forward_batch.num_token_non_padded_cpu=}", flush=True)

                        req_blocks = self.forward_metadata.block_tables[0]
                        tmp_k = k_cache[req_blocks, ...].reshape(-1, layer.tp_k_head_num * layer.qk_head_dim)[:max_seq_len]
                        tmp_v = v_cache[req_blocks, ...].reshape(-1, layer.tp_k_head_num * layer.qk_head_dim)[:max_seq_len]

                        print(f"+++ {rank=}, {layer.layer_id=}, {tmp_k.sum()=}, {tmp_v.sum()=}, {tmp_k[:, :5]=}, {tmp_v[:, :5]=}", flush=True)
                    ...
                    if not forward_batch.forward_mode.is_idle():
                        print(f"+++ {rank=}, {layer.layer_id=}, {query.shape=}, {k_cache.shape=}, {v_cache.shape=}, {self.forward_metadata.block_tables.shape=}"
                            f", {self.page_size=}, {layer.tp_k_head_num=}, {actual_seq_len_kv=}, {kv_dequant_scale.shape=}", flush=True)
                    # torch_npu.npu_fused_infer_attention_score.out(
                    #     query,
                    #     k_cache,
                    #     v_cache,
                    ...
                    if not forward_batch.forward_mode.is_idle():
                        print(f"+++ {rank=}, {layer.layer_id=}, before attention {q.sum()=}, {q.shape=}")
                    attn_output, _ = torch.ops.npu.npu_fused_infer_attention_score(
                        q.view(
                            forward_batch.batch_size,
                            -1,
                ...
                if not forward_batch.forward_mode.is_idle():
                    print(f"+++ {rank=}, {layer.layer_id=}, after attention {attn_output.sum()=}, {attn_output.shape=}, {attn_output[:, :, :5]=}", flush=True)
            # there are some accuracy issues in cross attention scene to use torch_npu._npu_flash_attention_qlens
            # forward_batch.encoder_lens is not None in cross attention scend, we add native attn to solve accuracy issues
                ...
                if not self.use_alibi:
                    ...
                    if not forward_batch.forward_mode.is_idle():
                        print(f"+++ {rank=}, {layer.layer_id=}, before attention {query.sum()=}, {query.shape=}")
                    ...
                    if not forward_batch.forward_mode.is_idle():
                        # if attn_output.sum().isnan():
                        #     raise ValueError("attention output is None")
                        print(f"+++ {rank=}, {layer.layer_id=}, after attention {attn_output.sum()=}, {attn_output.shape=}")
                else:
                    attn_output = self.attn_alibi(
                        q=query,
                        k_cache=k_cache,
